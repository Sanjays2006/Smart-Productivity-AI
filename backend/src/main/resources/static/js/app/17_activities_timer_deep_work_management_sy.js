...
window.confirmEnd = async function() {
    ...
    try {
        ...
        showToast('Could not save this session — it may be stale. Resetting the timer.', 'error', 5000);

        // Escape hatch: a failed save (often a stale/corrupt session) must not trap the
        // user in the "Log Completed Session" state forever. Reset locally so they can
